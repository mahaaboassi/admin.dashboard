...
import { useEffect } from 'react';
  const DropzoneFormInput = ({
    ...
    useEffect(()=>{onFileUpload(selectedFiles)},[selectedFiles])
    return <>
        {label && <FormLabel className={labelClassName}>{label}</FormLabel>}

        <Dropzone multiple={isMultiple} onDrop={acceptedFiles => handleAcceptedFiles(acceptedFiles, onFileUpload)}
                        ...
                        <div className="p-2">
                          <Row className="align-items-center">
                            {file.preview ? <Col xs={'auto'}>
                                <Image height={60} width={50} data-dz-thumbnail="" className="avatar-sm rounded bg-light" alt={file.name} src={file.preview} />
                              </Col> : <Col xs={'auto'}>
